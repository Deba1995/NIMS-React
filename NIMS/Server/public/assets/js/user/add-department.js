...
const form = document.querySelector("#formDepartment");
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch("/user/add-department", {
      method: "POST",
      body: dataFormat,
      headers: {
...
function cancelItem(id) {
  ...
  document.getElementById(`edit_${id}departmentName_err`).textContent = "";
  document.getElementById(`edit_${id}departmentName_err`).style.display =
    "none";
  document.querySelector(
    `button[data-id="${id}"][data-action="edit"]`
  ).style.display = "inline";
...
async function saveItem(id) {
  ...
  formData.append("departmentName", newName);
  const dataFormat = new URLSearchParams(formData);

  try {
    const res = await fetch(`/user/add-department/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
